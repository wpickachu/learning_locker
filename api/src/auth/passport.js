...
import { OAuth2Strategy as COAuth2Strategy } from 'passport-oauth2';
...
import { AUTH_OAUTH2_CALLBACK, AUTH_JWT_GOOGLE_CALLBACK } from 'lib/constants/routes';
...
T
passport.use('COAuth2', 
  new COAuth2Strategy({
    tokenURL: process.env.OAUTH2_TOKEN_URL,
    authorizationURL: process.env.OAUTH2_AUTHORIZATION_URL,
    clientID: process.env.OAUTH2_CLIENT_ID,
    clientSecret: process.env.OAUTH2_CLIENT_SECRET,
    callbackURL: `${process.env.SITE_URL}/api/${AUTH_OAUTH2_CALLBACK}`
  },
  function(accessToken, refreshToken, profile, done) {
    User.findOrCreate({ _id: profile.id }, (err, user) => {
      assert.ifError(err);
